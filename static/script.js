          ...
          <div class="history-group">
            ...
            ${groupSessions.map(sess => `
              <div class="conversation-history-item ${this.activeConversationId === sess.id ? 'active' : ''}" 
                ...
                <h4>${this.truncateMessage(sess.title || 'Untitled Conversation', 40)}</h4>
                <p class="conversation-meta">${sess.message_count} message${sess.message_count !== 1 ? 's' : ''} • ${this.formatChatTime(sess.updated_at)}</p>
    ...
    try {
      ...
      if (!this.activeConversationId) {
        ...
        if (sessionResponse.ok) {
          const sessionData = await sessionResponse.json();
          this.activeConversationId = sessionData.session_id;
          console.log('Created new session for file upload:', this.activeConversationId);
        } else {
      ...
      let uploadMessage = `📄 File "${data.filename}" uploaded successfully.`;
      ...
      if (data.analyses && data.analyses.length > 0) {
        ...
        data.analyses.forEach(analysis => {
          if (analysis.type === 'vision_analysis') {
            uploadMessage += `\n\n🔍 **Image Analysis:**\n${analysis.content.substring(0, 300)}${analysis.content.length > 300 ? '...' : ''}`;
          } else if (analysis.type === 'text_analysis') {
      ...
      this.renderMessage(uploadMessage, 'ai');
